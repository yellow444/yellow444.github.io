...
(function () {
  ...
  /** Расширенные описания навыков — data-skill → { desc, tags, level (1-5), details } */
  const SKILLS = {
    csharp: {
  ...
  const LEVEL_LABELS = ["", "Базовый", "Начальный", "Уверенный", "Продвинутый", "Экспертный"];
  ...
  function openSkillModal(card) {
    ...
    if (data && data.level) {
      const lv = data.level;
      let dots = '<span class="skill-modal__level-bar">';
      for (let i = 1; i <= 5; i++) {
        dots += '<span class="skill-modal__level-dot' + (i <= lv ? " filled" : "") + '"></span>';
      }
      dots += "</span>";
      levelEl.innerHTML = LEVEL_LABELS[lv] + " " + dots;
    }
